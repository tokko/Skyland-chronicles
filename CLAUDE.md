...
| Scene writer     | Sonnet 4.6 (default) — A/B with Sonnet 3.5 on scene-02 (see Pipeline parameters) | `Agent(model: "sonnet", subagent_type: "scene-writer", ...)` for 4.6; `Agent(subagent_type: "scene-writer-3-5", ...)` for 3.5 (model pinned in frontmatter — omit `model` parameter on the call) | Drafts and revises one scene from a brief. |
...
- **Pending model A/B test (scene-02).** Scene-02 of book-1 chapter-01 is the one-shot A/B comparison between Sonnet 4.6 (current scene-writer default) and Sonnet 3.5 (`claude-3-5-sonnet-20241022`, via the `scene-writer-3-5` agent). When scene-02 first drafts, the orchestrator spawns both writers in parallel against the same brief; outputs go to `draft-1-A.md` and `draft-1-B.md` (blind labels; the orchestrator records the mapping in a side file). The reviewer reads both blind, scores both against the same review criteria, and picks a winner. The winner becomes the canonical `draft-1.md` and continues through the standard pipeline. The losing draft is archived. **After scene-02 the winning model is locked in for all subsequent scenes**; update the scene-writer agent's frontmatter (or which sibling agent is invoked) accordingly, and note the decision in `STATUS.md`. The A/B runs once, on scene-02 only, and is not repeated.
