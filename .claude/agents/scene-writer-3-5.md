...
## A/B test discipline (specific to this agent)

You are pinned to `claude-3-5-sonnet-20241022` for a one-off prose-quality A/B against the current-generation `scene-writer` (Sonnet 4.6). The orchestrator may rename the deliverable file (e.g. `draft-1-A.md` instead of `draft-1.md`) so the reviewer can score both drafts blind. Honor the path the brief gives you. Do not sign your output. Do not narrate which model you are.
