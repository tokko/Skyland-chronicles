...
- **Stage**: **DRAINING — scene-02 substantial revision pass (draft-2) reviewed `minor`, edits applied, promoted.** Draft-1 (promoted earlier this session at 1,700 words) was re-opened on user canary feedback identifying six structural problems and one register problem (shield off-page, single-room scope, missing armour-saves, missing dispenser-room ambient cast, Anton not dying, "Drop it / you can walk" surrender-offers, register too light). Draft-2 written by Sonnet 4.6 against a comprehensive Revise-brief at 3,241 words; reviewer returned `minor` with twelve inline edits (ten em-dash conversions, one typo "grandmother was time" → "grandmother's time", two §3.1 thesis tightens). Twelve more em-dashes the reviewer's count missed were also brought down on a verify pass. Final scene-02.md: 3,486 words; em-dash count 2 (lines 127 and 135, both earned).
- **Active scene**: none — scene-02 promoted on the revision pass; scene-03 (*Raynard at the window*) is the next card up
- **Last completed scene**: book-1 / chapter-01 / scene-02 — *The Shopfloor* (3,486 words; reviewer verdict `minor` on draft-2; codex state-logs updated; Anton now dead in upper corridor)
